...
private:
    ...
    void InitializeServos() {
        ...
        // 初始化 4 路舵机通道
        gpio_num_t servo_pins[] = {SERVO1_PWM_PIN, SERVO2_PWM_PIN, SERVO3_PWM_PIN, SERVO4_PWM_PIN};
        for (int i = 0; i < 4; i++) {
            if (servo_pins[i] == GPIO_NUM_NC) continue;
            ledc_channel_config_t ch_cfg = {};
            ch_cfg.gpio_num = servo_pins[i];
            ch_cfg.speed_mode = LEDC_LOW_SPEED_MODE;
            ch_cfg.channel = (ledc_channel_t)(LEDC_CHANNEL_0 + i);
            ch_cfg.timer_sel = LEDC_TIMER_1;
            ch_cfg.duty = 0;
            ch_cfg.hpoint = 0;
            ESP_ERROR_CHECK(ledc_channel_config(&ch_cfg));
        }
        ESP_LOGI(TAG, "舵机初始化完成 (4通道 PWM / 50Hz)");
    }
    ...
    void InitializeMotors() {
        ...
        io_cfg.pin_bit_mask = (1ULL << MOTOR_A_IN1_PIN) | (1ULL << MOTOR_A_IN2_PIN) |
                              (1ULL << MOTOR_B_IN1_PIN) | (1ULL << MOTOR_B_IN2_PIN);
        io_cfg.pull_down_en = GPIO_PULLDOWN_DISABLE;
        io_cfg.pull_up_en = GPIO_PULLUP_DISABLE;
        io_cfg.intr_type = GPIO_INTR_DISABLE;
        ESP_ERROR_CHECK(gpio_config(&io_cfg));
        ESP_LOGI(TAG, "电机引脚初始化完成");
    }
